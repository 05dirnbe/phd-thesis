...
\section{Natural computing with \P}
	...
	\subsection{Key experiments and observations}
		...
		\subsubsection{Emergence of synchronization in \P }
			...
			\begin{figure}
				\centering
				\subfloat[Oscillator experiment - Thickness oscillations visualized][]{\includegraphics[width=\linewidth,keepaspectratio]{oscillator/thickness_oscillations.png}}
				\newline
				\subfloat[Oscillator experiment - Thickness oscillations over time][]{\includegraphics[width=\linewidth,keepaspectratio]{oscillator/thickness_oscillations_graph.png}}
		...
		\subsubsection{Shortest path in a maze}
			...
			\todo[inline]{make the explanation concise yet cover everything. Currently still missing are: time scale of the experiment}
			\todo[inline]{explain the motivation of the experiment. I.e. some people showed that oscillators can navigate a maze. Nakagaki studies \P as oscillators at the time. Thus, he puts it into the maze. References are in the thesis: physarum learner.}
		...
		\subsubsection{Maximizing food uptake through efficient networks connecting multiple food sources}
			...
			Experimentally, Nakagaki \etal find that the networks formed by \P resemble minimum Steiner trees with additional cycles which enhance fault tolerance, see \Fref{fig:steiner_tree_experiment}. The authors allow however, that the approximate Steiner points are likely a byproduct of searching for short connections between food sources. The inherent capability of forming functional networks was demonstrated impressively in another experiment where \P was set up to approximate the railway area of Tokyo and the Kansai area~\cite{tero2010rules}.

			\begin{figure}
				\centering
				\subfloat[Network formation experiment - Approximate MST][]{\includegraphics[width=0.3\linewidth,keepaspectratio]{mst/mst_a.png}}
				\subfloat[Network formation experiment - Approximate MST with additional cycles][]{\includegraphics[width=0.3\linewidth,keepaspectratio]{mst/mst_b.png}}
				\subfloat[Network formation experiment - Optimal MST][]{\includegraphics[width=0.3\linewidth,keepaspectratio]{mst/mst_c.png}}
	...
	\subsection{Natural computing approaches}
		...
		\subsubsection{Computing inspired by \P}
			...
			Due to its success and intuitive basic idea, the physarum solver has received much interest in the scientific community. Subsequently it inspired several other algorithms with varying degree of similarity to the original problem. A selection is given in \Fref{tab:list_inspired}. 

			\todo[inline]{present a list of problems for which a solution based on the physarum solver exists}
		...
		\subsubsection{Synthesis of \P using computation}
			...
			Given suitable parameters settings, the collective behavior of all agents leads to the spontaneous formation of transport networks. The emergent global patterns formed by the collective of agents resemble the actin-myosin mesh structure that form the veins of \P, while the individual trajectories of of the agents can be interpreted as the fluid flow in through the veins of the organism, see \Fref{fig:agent_structures}. \Fref{fig:agent_evolution} shows how an initially randomly distributed collection of agents evolves in time to eventually form a network. In particular the model can be tuned such that the collective demonstrates minimization processes observed in~\cite{jones2011influences,jones2015applications,baumgarten2015network}. 

			\begin{figure}
				\centering
				\subfloat[Oscillator experiment - Plasmodium][]{\includegraphics[width=0.45\linewidth,keepaspectratio]{agents/agent_structure.png}}
				\subfloat[Oscillator experiment - Plasmodium][]{\includegraphics[width=0.45\linewidth,keepaspectratio]{agents/agent_flow.png}}
			...
			\begin{figure}
				\centering
				\subfloat[Oscillator experiment - Plasmodium][]{\includegraphics[width=\linewidth,keepaspectratio]{agents/evolution.png}}
			...
			\todo[inline]{fix citation in this section.}
		...
		\subsubsection{Computing with live \P}
			...
			Thus, the exploring slime mold influences itself through the illumination pattern as it looks to occupy as large an area of possible. Under these conditions the plasmodium is lead towards certain configurations with a stable illumination pattern, see figure. 
			...
			\todo[inline]{fix the references and figures of this section \dots}

		\todo[inline]{find out in which natural computing category adamatzkys reaction diffusion stuff fits \dots}
