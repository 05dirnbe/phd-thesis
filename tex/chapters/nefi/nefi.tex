...
\section{Network Extraction From Images}	%	2298 words
	...
	\begin{figure}
		\centering
		\subfloat[NEFI's pipeline - Input image][Input image.]{\includegraphics{pipeline/input.jpg}\label{fig:pipeline:input}}
		\subfloat[NEFI's pipeline - Segmented image][Segmented image.]{\includegraphics{pipeline/segmented.jpg}\label{fig:pipeline:segmented}}
		\subfloat[NEFI's pipeline - Skeletonized image][Skeletonized image.]{\includegraphics{pipeline/skeleton.jpg}\label{fig:pipeline:skeleton}}
		\subfloat[NEFI's pipeline - Detected graph][Detected graph drawn ontop of the input.]{\includegraphics{pipeline/full_graph.jpg}\label{fig:pipeline:full_graph}}
		\subfloat[NEFI's pipeline - Filtered graph][Filtered graph drawn ontop of the input.]{\includegraphics{pipeline/largest_component.jpg}\label{fig:pipeline:largest_component}}
	...
	\begin{figure}
		\centering
		\includegraphics[width=\textwidth]{gui.jpeg}
	...
	\begin{figure}
		\centering
		\subfloat[\P - input to NEFI][]{\includegraphics{p_polycephalum.jpg}\label{fig:physarum:input}}
		\subfloat[\P - output of NEFI][]{\includegraphics{p_polycephalum_graph.jpg}\label{fig:physarum:graph}}
	...
	\centering
		\begin{tikzpicture}
			...
			\node {\includegraphics[width=\textwidth]{dragonfly_optimal.jpeg}};
