...
\section{Repository Concept and Benefits}
	...
	The \SMGR and all available data can be found here: \href{http://smgr.mpi-inf.mpg.de}{smgr.mpi-inf.mpg.de}. 
...
\section{The KIST Europe Data Set}	
	...
	\begin{enumerate}
		\item A rectangular plastic dish is prepared with a thin sheet of \SI[round-mode=places,round-precision=2]{1.25}{\percent} agar.
		\item \SI[round-mode=places,round-precision=1]{1.5}{\gram} of dried \P (HU195xHU200) sclerotia crumbs are lined up along the short edge of the dish, see \Fref{fig:sequence_1}. The dish is put into a large light-proof box.
...
\section{Sample Usage of the KIST Europe Data Set}
	...
	\begin{figure}[!htp]
		...
		\caption[Demo - Computing an observable across an entire series of graphs]{(a) A sample graph of \emph{P. polycephalum} from the \data. It is part of a series of $60$ graphs. The area indicated by the dashed rectangle is magnified in \Fref{fig:split_examples} (b) Cumulative edge width distribution computed over all $60$ graphs.}
    \end{figure}


	While computing such histograms is merely an exercise of using the information stored in the graphs, more interesting and insightful observables can be derived. In fact, in \Fref{chap:analysis} the results of a comprehensive non-trivial study of the \data will be presented~\cite{dirnberger2016}.
	...
	\begin{equation}
		\text{color}(e) = \{\text{color}(u), \text{color}(v)\} \,.
	\end{equation}
	...
	\begin{figure}
		...
		\caption[Demo - Setup for tracking individual edges]{Detail of a \emph{P. polycephalum} graph, corresponding to the dashed rectangle in \Fref{fig:graph}. (a) Two edges $A$ and $B$ are selected for tracking. (b) A third edge appears, causing $B$ to split up causing it to disappear from its track.}
