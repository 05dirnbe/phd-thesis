...
\chapter{Slime Mold Graph Repository}\label{chap:smgr}
	...
	The present chapter reports joined work with Mag.~T.~Mehlhorn~\cite{SMGR}. He was responsible for the realization of all described wet-lab experiments in the laboratories of the Korea Institute of Science and Technology Europe (\href{www.kist-europe.de}{\textsc{Kist} Europe}).
...
\section{The KIST Europe Data Set}	
	...
	\subsection{Node Tracking}
		...
		For each of our experiments, let us denote the respective time ordered sequence of $k$ filtered graphs with $\mathcal{G} = {G_1, G_2, \dots, G_t, \dots, G_k}$ and the union of the respective node sets with $\bar{V} = \bigcup_{i=1}^{k} V_{k}$. Each node in $\bar{V}$ can be represented as a non-negative integer triple $(x,y,t)$. Here $x$ and $y$ denote a node's pixel coordinates relating to the input image used for graph extraction and $t$ denotes its position in the sequence, \ie its position in time.
...
\section{Sample Usage of the KIST Europe Data Set}
	...
	\begin{figure}
		...
		\caption[Demo - Setup for tracking individual edges]{Detail of a \emph{P. polycephalum} graph, see dashed rectangle in \Fref{fig:graph}. (a) Two edges $A$ and $B$ are selected for tracking. (b) A third edge appears, causing $B$ to split up causing it to disappear from its track.}
