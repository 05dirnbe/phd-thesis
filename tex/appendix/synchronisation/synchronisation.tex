...
\captionof{listing}[Model implementation - Required libaries]{Required python libaries.  Here the excellent \mintinline{python}{networkx} provides graph support~\cite{networkx}.}
\label{code:libaries}
...
\begin{codesnippet}
\captionof{listing}[Model implementation - Update capacitor voltages]{Function updating the capacitor voltages for all edges. The quantity \mintinline{python}{Ucs} is the sum over all capacitor voltages and invariant by \Fref{lem:inv_cont} since $C_e = C$ for all $e \in G$ as determined by the intital conditions.}
