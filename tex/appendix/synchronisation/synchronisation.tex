...
import numpy as np
\end{minted}
\captionof{listing}{Required python libaries.  Here the excellent \mintinline{python}{networkx} provides graph support~\cite{schult2008exploring}.}
\label{code:libaries}
\end{codesnippet}
...
def simulation(graph, epsilon = 0.01, max_iterations = 3000):
	...
	return graph
\end{minted}
\captionof{listing}{Setup of the simulation. Note that the sequence of updates is different from \Fref{sec:discrete}. This change simplifies computation but does not affect the results.}
...
\end{minted}
\captionof{listing}{Function updating the voltages for all nodes. Implements \Fref{eq:uv}. Using the dictionaries \mintinline{python}{R_map} and \mintinline{python}{R_map} in conjunction with \mintinline{python}{np.prod} and \mintinline{python}{combinations} yields a convenient way to compute the required sums and products.}
...
\captionof{listing}{Function updating the capacitor voltages for all edges. The quantity \mintinline{python}{Ucs} is the sum over all capacitor voltages and invariant by \Fref{lem:inv_cont} since $C_e = C \quad \forall e \in E$ as determined by the intital conditions.}
...
\captionof{listing}{Updates for current controlled voltage source. Implements \Fref{eq:left_up_18}, \Fref{eq:left_up_19} and \Fref{eq:left_up_20} for the l.h.s and \Fref{eq:right_up_21}, \Fref{eq:right_up_22} and \Fref{eq:right_up_23} for r.h.s voltage source respectively.}
