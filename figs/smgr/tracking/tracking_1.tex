...
\begin{document}
    ...
    \begin{tikzpicture}[scale=1.8, auto,swap]
        ...
        \foreach \source/ \dest /\weight in {$c_1$/$b_1$/7, $a_1$/$b_1$/2, $e_1$/$d_1$/1, $e_1$/$f_1$/2,$e_1$/$b_1$/3}
            \path[edge] (\source) -- node[weight] {} (\dest);
       
        \node[draw] at (2.2,-0.4) {\LARGE $t=1$};
